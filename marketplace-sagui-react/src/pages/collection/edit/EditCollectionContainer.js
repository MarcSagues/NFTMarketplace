...
export default function EditCollectionContainer() {
  ...
  const [url, setUrl] = useState("https://sagui-market.web.app/collection/");
  ...
  const handleChangeURL = (value) => {
    setUrlError(false);
    let finalValue = value.split("https://sagui-market.web.app/collection/");

    if (finalValue[1]) {
      setUrl(`https://sagui-market.web.app/collection/${finalValue[1]}`);
    } else {
      setUrl("https://sagui-market.web.app/collection/");
    }
  };
  ...
  const handleCreateCollection = async () => {
    ...
    if (!checkURLFormat("https://sagui-market.web.app/collection/", url)) {
      error = true;
      setUrlError(true);
      setUrlErrorMessage("El formato de URL es incorrecto");
    }

    let customURL = url.split("https://sagui-market.web.app/collection/")[1];
  ...
  const getCollectionDetails = async () => {
    ...
    setUrl(
      `https://sagui-market.web.app/collection/${collectionDetail.customURL}`
    );
          ...
          <div className="flex flex-col w-full pt-[40px] content-center justify-left">
            <div className="flex flex-row gap-2">
              ...
              <HelpTooltip
                ...
                tooltipText="Si el contenido és explícito o sensible, como pornografía <br/> o contenido 'not safe for work' (NSFW),  protegerá a los usuarios <br/> de SAGUI que realicen búsquedas seguras y no les mostrará el contenido."
              />
            </div>
          </div>
