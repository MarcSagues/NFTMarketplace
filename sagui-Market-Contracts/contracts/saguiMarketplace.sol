...
interface ISaguiAddressRegistry {
    function saguiCollection() external view returns (address);
    ...
    ERC2771ContextUpgradeable
{
    ...
    ISaguiAddressRegistry public addressRegistry;

    ISaguiVerification saguiVerification;
    ...
    modifier isVerifiedAddress(address _address) {
        bool isValidAddress = saguiVerification.checkIfVerified(_address);
        if (!isValidAddress) {
            bool isValidInversor = saguiVerification.checkIfVerifiedInversor(
                _address
            );
    ...
    function createOffer(
        ...
        ISaguiAuction auction = ISaguiAuction(addressRegistry.auction());
    ...
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = ISaguiAddressRegistry(_registry);
    }

    function updateSaguiVerification(address _verification) external onlyOwner {
        saguiVerification = ISaguiVerification(_verification);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    ISaguiTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
