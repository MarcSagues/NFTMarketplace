...
async function main(network) {
  ...
  const Marketplace = await ethers.getContractFactory("SaguiMarketplace");
  const marketplaceImpl = await Marketplace.deploy(FORWARDER);
  await marketplaceImpl.deployed();

  console.log("SaguiMarkeplace deployed to: ", marketplaceImpl.address);
  ...
  //// Auction deployement
  const Auction = await ethers.getContractFactory("SaguiAuction");
  const auctionIml = await Auction.deploy(FORWARDER);
  await auctionIml.deployed();

  console.log("SaguiMarkeplace deployed to: ", auctionIml.address);
  ...
  const auction = await ethers.getContractAt("SaguiAuction", AUCTION_ADDRESS);
  ...
  //// Factory deployement
  const Factory = await ethers.getContractFactory("SaguiArtFactory");
  const factoryImpl = await Factory.deploy(FORWARDER);
  await factoryImpl.deployed();

  console.log("SaguiArtFactory deployed to: ", factoryImpl.address);
  ...
  console.log("SaguiArtFactory Proxy deployed at: ", factoryProxy.address);
  ...
  console.log("SaguiArtFactory Proxy Initialized");
  ////

  //// TokenRegistry Deployement
  const TokenRegistry = await ethers.getContractFactory("SaguiTokenRegistry");
  ...
  const Community = await ethers.getContractFactory("SaguiCommunity");
  const comunityImpl = await Community.deploy();
  await comunityImpl.deployed();

  console.log("SaguiCommunity deployed to: ", comunityImpl.address);
  ...
  const Verification = await ethers.getContractFactory("SaguiVerification");
  ...
  await marketplace.updateSaguiVerification(VERIFICATION_ADDRESS);
  await community.updateSaguiVerification(VERIFICATION_ADDRESS);
  await factory.updateSaguiVerification(VERIFICATION_ADDRESS);
