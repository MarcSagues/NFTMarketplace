...
const SaguiNFT = artifacts.require("DefaultSagui");
const SaguiMarketplace = artifacts.require("SaguiMarketplace");
const SaguiVerification = artifacts.require("SaguiVerification");

contract(
  "Core ERC721 tests for SaguiNFT and Marketplace",
  function ([owner, minter, buyer, feeRecipient]) {
    ...
    beforeEach(async function () {
      this.marketplace = await SaguiMarketplace.new(feeRecipient, platformFee, {
        from: owner,
      });

      this.nft = await SaguiNFT.new(this.marketplace.address, { from: owner });
      this.verification = await SaguiVerification.new({ from: owner });

      this.nft.updateSaguiVerification(this.verification.address, {
        from: owner,
      });

      this.marketplace.updateSaguiVerification(this.verification.address, {
        from: owner,
      });
